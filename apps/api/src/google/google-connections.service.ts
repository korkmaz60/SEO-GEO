...
export class GoogleConnectionsService {
  ...
  async status(workspaceId: string): Promise<GoogleIntegrations> {
    ...
    return { configured: this.google.configured, connections: rows.map(toConnection) };
  }
  ...
  async authorize(
    ...
    return { url: this.google.authorizationUrl({ state, codeChallenge: challenge }) };
  }
  ...
  async callback(
    ...
    try {
      exchanged = await this.google.exchangeCode(query.code, state.verifier);
    } catch {
    ...
    const refreshToken =
      tokens.refreshToken ?? (existing ? this.openTokens(existing).refreshToken : null);
  ...
  async accessToken(connection: ConnectionRow): Promise<string> {
    ...
    try {
      tokens = await this.google.refresh(stored.refreshToken);
    } catch (error) {
      if (error instanceof GoogleApiError && error.revoked) {
        await this.prisma.googleConnection.update({
          where: { id: connection.id },
          data: { status: "REVOKED", lastError: error.message.slice(0, 500) },
        });
