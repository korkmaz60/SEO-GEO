...
describe("api application", () => {
  ...
  beforeAll(async () => {
    const config = loadConfig({ NODE_ENV: "test", APP_VERSION: "9.9.9" });
  ...
  it("serves the health endpoint under /v1", async () => {
  ...
  it("returns problem+json for unknown routes", async () => {
    const response = await request(app.getHttpServer()).get("/v1/nope?token=secret").expect(404);
    ...
    expect(problem).toMatchObject({
      ...
      code: "not_found",
      instance: "/v1/nope",
      detail: "No route for GET /v1/nope",
    });
  ...
  it("publishes the OpenAPI document", async () => {
    const response = await request(app.getHttpServer()).get("/v1/openapi.json").expect(200);

    expect(response.body.openapi).toMatch(/^3\./);
    expect(response.body.info).toMatchObject({ title: "SEO-GEO API", version: "9.9.9" });
    expect(Object.keys(response.body.paths)).toContain("/v1/health");
  });
});
