...
| D22 | Domain and backlink data loaded on demand from DataForSEO Labs and the Backlinks API (pay-as-you-go since July 2026), shared by all workspaces through the provider cache (7 days, see D23), with a quote before every paid load and no scheduled snapshots | Accepted (M4) |
| D23 | Domain and backlink data stay cached for 7 days: backlink profiles change slowly, and day-by-day new and lost links come from DataForSEO's own series. Pages show when the data was fetched and offer a Refresh that loads it again after showing its cost; the new response replaces the cached one for every workspace | Accepted (M4) |
| D24 | MCP clients can also sign in with OAuth 2.1 as the MCP authorization spec describes (discovery metadata, dynamic client registration or client ID metadata documents, PKCE), so Claude's custom connectors and other hosted assistants connect without an API key; the consent page picks one workspace and an access level that become the token's scopes (as in D19); API keys stay for scripts and clients that send a header | Proposed |
| D25 | Page experience from Google's free APIs rather than a browser of our own: Core Web Vitals field data from the Chrome UX Report and Lighthouse lab audits through the PageSpeed Insights API; field and lab values are shown apart (INP only from field data, TBT stays a lab metric), and a site with too little traffic shows "not enough real-user data" | Proposed |
